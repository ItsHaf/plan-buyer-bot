...
export class DropController {
      ...
      const checkPromises = Array.from(this.workers.values())
        ...
        .map(async (worker) => {
          ...
          if (this.isOnTargetSite(currentUrl)) {
            const isHighTraffic = await this.checkForHighTrafficState(worker);
            if (isHighTraffic) {
              console.log(`[Tab ${worker.id}] High-traffic state, refreshing...`);
    ...
    while (!this.successFlag && !this.isShuttingDown) {
      ...
      const checkPromises = Array.from(this.workers.values())
        ...
        .map(async (worker) => {
          ...
          if (this.isOnTargetSite(currentUrl)) {
            // Check for high-traffic state — refresh if detected
            const isHighTraffic = await this.checkForHighTrafficState(worker);
            if (isHighTraffic) {
              console.log(`[Tab ${worker.id}] High-traffic state, refreshing...`);
              await this.refreshTab(worker);
  ...
  private async navigateWithRetry(worker: TabWorker): Promise<void> {
    ...
    while (!success && !this.isShuttingDown) {
      ...
      try {
        ...
        const hasHighTraffic = await this.checkForHighTrafficState(worker);

        if (hasHighLoadError || hasHighTraffic) {
          console.log(`[Tab ${worker.id}] ⚠️ Rate limit / high traffic, retrying...`);
  ...
  /**
   * Check if the page shows the high-traffic blocking state.
   * The site now shows a top banner "当前购买人数较多，请稍后重试" and
   * buy buttons display "抢购人数过多，请刷新再试" instead of being real buy buttons.
   * Returns true if either indicator is present (page needs refreshing).
   */
  private async checkForHighTrafficState(worker: TabWorker): Promise<boolean> {
    try {
      // Check for top banner: "当前购买人数较多，请稍后重试"
      const bodyText = await worker.page.locator('body').innerText().catch(() => '');
      if (bodyText.includes('当前购买人数较多') || bodyText.includes('当前访问人数较多')) {
        console.log(`[Tab ${worker.id}] High-traffic banner detected`);
        return true;
      }

      // Check buy button text: "抢购人数过多，请刷新再试"
      const buyButtons = worker.page.locator('.buy-btn');
      const count = await buyButtons.count().catch(() => 0);
      for (let i = 0; i < count; i++) {
        const btnText = await buyButtons.nth(i).textContent().catch(() => '');
        if (btnText && btnText.includes('抢购人数过多')) {
          console.log(`[Tab ${worker.id}] High-traffic button text detected`);
          return true;
        }
      }
  ...
  /**
   * Check if current page is the rate limit error page (old full-page redirect)
   */
  ...
  private async refreshTab(worker: TabWorker): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        const currentUrl = worker.page.url();
        if (this.isOnTargetSite(currentUrl) && !currentUrl.includes('rate-limit.html')) {
          // Check if the page loaded but is still in high-traffic state
          const isHighTraffic = await this.checkForHighTrafficState(worker);
          if (isHighTraffic) {
            console.log(`[Tab ${worker.id}] Refreshed but still high-traffic`);
          } else {
   ...
   * - Tabs that look like they're on the target page but show high-traffic state
   *   (banner "当前购买人数较多" or buttons "抢购人数过多") should keep refreshing.
   * - Tabs cleanly on the target page with real buy buttons should stop mass-refreshing.
   */
  private async shouldRefreshWorkerAfterDrop(worker: TabWorker): Promise<boolean> {
    const currentUrl = worker.page.url();

    // Check payment first — always stop if we reached checkout
    await this.checkForPayment(worker);
    if (this.successFlag) {
      return false;
    }

    // If on what looks like the target page, check for high-traffic state
    if (this.isStableTargetPage(currentUrl)) {
      const isHighTraffic = await this.checkForHighTrafficState(worker);
      if (isHighTraffic) {
        console.log(`[Tab ${worker.id}] Page loaded but high-traffic state — will keep refreshing`);
        return true;
      }

      // No high-traffic — check for actual buy buttons
      if (!worker.buyButtonDetected) {
        await this.checkForBuyButton(worker);
      }
    ...
    // First post-drop refresh is mandatory unless already actionable
    if (worker.postDropRefreshCount === 0) {
      console.log(`[Tab ${worker.id}] First post-drop refresh`);
      return true;
    }

    // Keep refreshing if not on a stable target page
    if (!this.isStableTargetPage(currentUrl)) {
      return true;
    }
